...
class User_all_templates extends CI_Controller {
    ...
    private $controller = 'User_all_templates';
    ...
    public function index()
    {
        ...
        $data['title_page'] = 'All Template | Goodeva';
        ...
        $id_type = $this->input->get('type_template');


        $data['data_kategori'] = $this->Model_kategori->get_data()->result();
        $data['type_template'] = $this->db->query('SELECT * from table_type_template')->result();


        if($this->input->get('kategori') != 0 && $this->input->get('type_template') == 0 && $this->input->get('nama_template') == '' ){
        // Cari berdasarkan Kategori
            $data['template_result'] = $this->Model_template->get_dataByKategori($id_kategori)->result();
            $data['total_template'] = $this->Model_template->get_dataByKategori($id_kategori)->num_rows();

        }else{
        // Cari berdasarkan Kategori dan Type
            $data['template_result'] = $this->Model_template->get_data()->result();
            $data['total_template']  = $this->Model_template->get_data()->num_rows();
        ...
        $this->load->view('vu_all_templates',$data);
